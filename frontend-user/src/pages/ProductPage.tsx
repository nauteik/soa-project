...
const ProductPage: React.FC = () => {
  // Hỗ trợ cả 2 định dạng URL: "/category/:slug" và "/:categorySlug"
  const { slug, categorySlug } = useParams<{ 
    slug?: string,          // Cho đường dẫn cũ /category/:slug
    categorySlug?: string   // Cho đường dẫn mới /:categorySlug
  }>();
  ...
  useEffect(() => {
    if (categorySlugToUse) {
      ...
      if (categories && categories.length > 0) {
        const foundCategory = categories.find(c => c.slug === categorySlugToUse);
        setCategory(foundCategory || null);
        setCategoryLoading(false);
      }
    }
  }, [categorySlugToUse, categories]);
  
  // Fetch products with debounce when filters change
  useEffect(() => {
    // Only fetch products when we have a valid slug
    if (!categorySlugToUse) return;
  ...
  }, [categorySlugToUse, loadMoreCount, selectedBrands, effectivePriceRange, selectedSpecs, sortOption]);

  // Fetch brands and specifications by categoryId and categorySlug
  useEffect(() => {
    if (!categorySlugToUse) return;
    ...
    const fetchFilters = async () => {
      try {
        let brandsRes: Brand[] = [];
        let specsRes: Record<string, string[]> = {};
        brandsRes = await getBrands();
        if (categorySlugToUse) {
          specsRes = await getSpecificationsByCategorySlug(categorySlugToUse);
        }
        setBrands(brandsRes);
  ...
  // Load specification fields từ API khi category thay đổi
    ...
    if (categorySlugToUse) {
      fetchCategorySpecifications(categorySlugToUse)
        ...
        .catch(error => {
          ...
          // Không thay đổi state spec fields khi lỗi (giữ nguyên giá trị trước đó nếu có)
          // setSpecFields([]);
        });
    }
  }, [categorySlugToUse]);
  
  // Tạo URL sử dụng định dạng mới
  ...
  // Format spec name for display - sử dụng specFields thay vì hardcode
  ...
  const SPEC_EXCLUDE = ['weight_kg', 'cpu_model'];
  // Prioritize these specs
  const SPEC_PRIORITY = ['usage_type', 'ram_gb', 'refresh_rate_hz', 'storage_gb'];
        ...
        {categoryPath.map((cat, index) => (
          <React.Fragment key={cat.id}>
            ...
            {index === categoryPath.length - 1 ? (
              ...
              <Link to={generateCategoryUrl(cat.slug)} className="hover:text-primary">
                {cat.name}
              </Link>
            )}
          </React.Fragment>
        ...
        `}>
          ...
          <div className="bg-white rounded-lg border md:sticky md:top-4 md:max-h-[calc(100vh-100px)] md:overflow-y-auto">
            ...
            {Object.keys(specifications).length > 0 ? (
              // Sort and filter specifications
              [
                ...SPEC_PRIORITY.filter(key =>
                  Object.prototype.hasOwnProperty.call(specifications, key) && !SPEC_EXCLUDE.includes(key)
                ),
                ...Object.keys(specifications)
                  .filter(key =>
                    !SPEC_PRIORITY.includes(key) && !SPEC_EXCLUDE.includes(key)
                  )
              ].map((specKey) => {
